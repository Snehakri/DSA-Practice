...
// {
//     public:
//     Node* solve(int in[],int pos[],int i,int j,int a,int b){
//         if(i>j) return NULL;
//         if(i==j) return new Node(in[i]);
//         Node *root=new Node(pos[b]);
//         int ind=-1;
//         for(int k=i;k<=j;k++){
//             if(in[k]==pos[j]){
//                 ind=i;
...
//         root->left=solve(in,pos,i,j-1,a,a+left-1);
//         root->right=solve(in,pos,ind+1,j,b-right,b-1);
//         return root;
//     }
